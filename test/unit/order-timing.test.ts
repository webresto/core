...
describe("order-timing", function () {
  ...
  const center = { lat: 56.8371, lon: 60.6019 };
  const north = { lat: 56.8907, lon: 60.6103 };
  const dishes = [
  ...
  describe("the whole promise", function () {
    ...
    it("has no floor when the adapter reported none: the road alone", async function () {
      const asked: string[] = [];
      (global as any).Settings = { async get(key: string) { asked.push(key); return ({ DELIVERY_CITY_SPEED_KMH: 60 } as any)[key]; } };
      const estimate = await estimateDeliveryTime({ products: dishes, kitchen: center, customer: north }, adapter);
      expect(estimate.travelMinutes).to.be.greaterThan(0);
      expect(estimate.totalMinutes).to.equal(20 + estimate.travelMinutes);
      // The zone is the only floor; there is no installation-wide one to fall back on.
      expect(asked).to.deep.equal(["DELIVERY_CITY_SPEED_KMH", "DELIVERY_SAFETY_MARGIN_MINUTES"]);
    });
