...
describe("Delivery zone import", function () {
  ...
  describe("terms of new rows", function () {
    const byExternalId = (externalId: string) => rows.find((row) => row.externalId === externalId);

    it("gives them to a new layer and a new loose zone, not to a zone its new layer prices", async function () {
      const layer = { externalId: "layer:North", name: "North" };
      await DeliveryZoneImportService.apply(
        snapshot([
          { externalId: "z1", name: "A", polygon: ring, layer },
          { externalId: "z2", name: "B", polygon: ring },
        ]),
        { terms },
      );

      expect(byExternalId("layer:North")).to.include(terms);
      expect(byExternalId("z2")).to.include(terms);
      expect(byExternalId("z1").minDeliveryTime).to.equal(undefined);
      expect(byExternalId("z1").deliveryCost).to.equal(undefined);
    });

    it("gives them to a new zone in an existing layer that only groups", async function () {
      rows = [{
        id: "l1", source: "kml", externalId: "layer:North", name: "North",
        termsApplyToZones: false, minDeliveryTime: 30, deliveryCost: 100,
      }];

      await DeliveryZoneImportService.apply(
        snapshot([{ externalId: "z1", name: "A", polygon: ring, layer: { externalId: "layer:North", name: "North" } }]),
        { terms },
      );

      expect(byExternalId("z1")).to.include(terms);
      expect(byExternalId("layer:North")).to.include({ minDeliveryTime: 30, deliveryCost: 100 });
    });

    it("never rewrites the terms of a row that already exists", async function () {
      rows = [{ id: "z1", source: "kml", externalId: "a", name: "A", polygon: ring, minDeliveryTime: 30, deliveryCost: 100 }];

      await DeliveryZoneImportService.apply(snapshot([{ externalId: "a", name: "renamed", polygon: ring }]), { terms });

      expect(rows[0]).to.include({ name: "renamed", minDeliveryTime: 30, deliveryCost: 100 });
    });
  });
  ...
  describe("zones out of a file", function () {
    ...
    it("gives every zone the terms stated with the file: a file has no layers to lend them", async function () {
      await importLocalZones({ city: null, zones: [{ name: "A", polygon: ring }, { name: "B", polygon: ring }], terms });

      expect(rows.every((row) => row.minDeliveryTime === 60 && row.deliveryCost === 250)).to.equal(true);
    });
