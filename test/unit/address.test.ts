...
import { formatAddressLine, formatAddressPath } from "../../lib/address";
import { locateAddress } from "../../lib/delivery-location";
...
describe("Address catalog", function () {
  ...
  const row = (values: Row): Row => ({
    parent: null, names: [], point: null, lo: null, hi: null, parity: "any", enable: true, ...values,
  });

  const rows: Row[] = [
    row({ id: "lenina", city: ekb, type: "street", name: "Ленина", names: ["Lenina"] }),
    row({ id: "malysheva", city: ekb, type: "street", name: "Малышева" }),
    row({ id: "lenina-12", city: ekb, parent: "lenina", type: "house", name: "12", point: { lat: 56.83, lng: 60.6 } }),
    row({ id: "lenina-14", city: ekb, parent: "lenina", type: "house", name: "14", point: { lat: 56.84, lng: 60.6 } }),
    row({ id: "lenina-12-p3", city: ekb, parent: "lenina-12", type: "entrance", name: "подъезд 3" }),
    row({ id: "hotel", city: ekb, type: "place", name: "гост. Прибалтийская", names: ["Baltic hotel"], point: { lat: 56.8, lng: 60.5 } }),
    row({ id: "hotel-cabin", city: ekb, parent: "hotel", type: "unit", name: "Домик 7" }),
    row({ id: "closed", city: ekb, type: "street", name: "Ленинградская", enable: false }),
    ...
    // Малышева: a block of odd numbers plus one house the catalog really has.
    row({ id: "malysheva-odd", city: ekb, parent: "malysheva", type: "range", name: "1–99, нечётные", lo: 1, hi: 99, parity: "odd", point: { lat: 56.836, lng: 60.614 } }),
    row({ id: "malysheva-145", city: ekb, parent: "malysheva", type: "house", name: "145", point: { lat: 56.8421, lng: 60.664 } }),
    ...
    row({ id: "tent-42", city: camp, type: "unit", name: "Шатёр 42", point: { lat: 57.1, lng: 60.1 } }),
  ...
  describe("ranges of house numbers", function () {
    ...
    it("offers the block a typed number falls into", async function () {
      expect(names(await under("45а"))).to.deep.equal(["1–99, нечётные"]);
    });

    it("says nothing when the parity is wrong", async function () {
      expect(await under("46")).to.deep.equal([]);
    });
  ...
  describe("where a node stands", function () {
    ...
    it("stands a node with no point of its own where its parent stands", async function () {
      ...
      expect(where.coordinate).to.deep.equal({ lat: 56.8, lng: 60.5 });
    ...
    it("prefers the node's own point to the one above it", async function () {
      ...
      expect(where.coordinate).to.deep.equal({ lat: 56.83, lng: 60.6 });
      expect(where.diagnostics.join(" ")).to.contain('address-node (house "12")');
    });

    it("stands a range at its own middle", async function () {
      const where = await locateAddress(noGeocoder, { node: "malysheva-odd", home: "45а" } as never);
      expect(where.coordinate).to.deep.equal({ lat: 56.836, lng: 60.614 });
    });
  });
  ...
  describe("what a node is allowed to be", function () {
    ...
    it("refuses a point that is not a point", async function () {
      const error = await create({ city: ekb, parent: "lenina", type: "house", name: "16", point: { lat: 200, lng: 0 } });
