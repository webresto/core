...
describe("Soft delivery calculation", function () {
  ...
  describe("both entry points", function () {
    ...
    const outside = { city: "Demo", formatted: "Far, 1", home: "1", coordinate: { lat: 50, lon: 50 } } as any;
    ...
    it("answers a dead geocoder the way checkout will answer", async function () {
      ...
      const calculated = await adapter.calculate({ address: typed, basketTotal: 1000 } as any);
      expect(calculated.allowed).to.equal(ability.allowed);
      expect(calculated.cost).to.equal(ability.cost);
    ...
    // Zones are the only tariff: no zone, no delivery — and nothing is read
    // from settings to price it instead.
    it("refuses delivery on an installation with no zones, on both entry points", async function () {
      (global as any).DeliveryZone = { findServing: async () => [] };
      invalidateDeliveryZoneCache();
      settings.SOFT_DELIVERY_CALCULATION = true;
      ...
      for (const delivery of [ability, calculated]) {
        expect(delivery.allowed).to.equal(false);
        expect(delivery.cost).to.equal(0);
        expect(delivery.message).to.equal("Delivery is not available");
    ...
    it("places an address it has no coordinate for as unrecognised once zones exist", async function () {
      settings.SOFT_DELIVERY_CALCULATION = false;
      // No point, no catalog node, no house number: nothing to geocode.
      const vague = { city: "Demo", formatted: "Somewhere" } as any;

      const ability = await adapter.checkAbility(vague);
      expect(ability.allowed).to.equal(false);
      expect(ability.message).to.equal("Coordinates not found");
    });
  });
