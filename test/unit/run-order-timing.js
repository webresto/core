...
const T = require(path.join(BUILD, "lib/order/order-timing.js"));
...
(async () => {
  ...
  // No floor reported: the road alone. The zone is the only floor there is.
  bindSettings({ DELIVERY_CITY_SPEED_KMH: 60 });
  e = await T.estimateDeliveryTime({ products: dishes, kitchen: center, customer: north }, adapter);
  assert.ok(e.travelMinutes > 0);
  assert.strictEqual(e.totalMinutes, 20 + e.travelMinutes + 0);
  passed++;
