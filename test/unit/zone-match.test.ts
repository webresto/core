...
describe("Delivery zone geometry", function () {
  ...
  describe("zone selection", function () {
    ...
    it("takes the first match, so sortOrder decides overlaps", function () {
      expect(findZoneForCoordinate([small, big], { lat: 10, lon: 10 })?.id).to.equal("small");
      expect(findZoneForCoordinate([big, small], { lat: 10, lon: 10 })?.id).to.equal("big");
    });

    it("returns null outside every zone", function () {
      expect(findZoneForCoordinate([small, big], { lat: 50, lon: 50 })).to.equal(null);
    });

    it("skips zones with an unusable polygon instead of throwing", function () {
      expect(findZoneForCoordinate([{ id: "broken", polygon: [[0, 0]] }, small], { lat: 10, lon: 10 })?.id)
        .to.equal("small");
    });
  });
  ...
  describe("kitchen in zone", function () {
    ...
    const inside = { id: "inside", coordinate: { lat: 10, lon: 10 } };
    const nearer = { id: "nearer", coordinate: { lat: 10.4, lon: 10.4 } };
    const outside = { id: "outside", coordinate: { lat: 10.6, lon: 12 } };
    ...
    it("breaks a tie by id", function () {
      const twinA = { id: "a", coordinate: { lat: 10, lon: 10 } };
      const twinB = { id: "b", coordinate: { lat: 10, lon: 10 } };
