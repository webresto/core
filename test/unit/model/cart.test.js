...
describe('Cart', function () {
    ...
    it('addDish', async function () {
        ...
        let result = await Cart_1.default.findOne(cart.id).populate('dishes');
    ...
    it('removeDish', async function () {
        let dish = (await Cart_1.default.findOne(cart.id).populate('dishes')).dishes[1];
    ...
    it('setCount', async function () {
        let dish = (await Cart_1.default.findOne({ id: cart.id }).populate('dishes')).dishes[0];
        dish = await CartDish.findOne(dish.id);
    ...
    it('returnFullCart', async function () {
        ...
        let res = await Cart_1.default.returnFullCart(cart);
    ...
    it('setSelfService', async function () {
        let cart = await Cart_1.default.create({});
        await cart.addDish(dishes[0], 5, [], '', '');
        await cart.addDish(dishes[1], 3, [], '', '');
        await cart.addDish(dishes[2], 8, [], '', '');
        await cart.setSelfService(true);
        let changedCart = await Cart_1.default.findOne(cart.id);
        chai_1.expect(changedCart.selfService).to.equal(true);
        await cart.setSelfService(false);
        changedCart = await Cart_1.default.findOne(cart.id);
    ...
    it('countCart', async function () {
        ...
        cart = await Cart_1.default.findOne(cart.id);
        await Cart_1.default.countCart(cart);
        let changedCart = await Cart_1.default.findOne(cart.id);
