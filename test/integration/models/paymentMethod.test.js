...
describe("PaymentMethod", function () {
    ...
    it("getAdapter", async function () {
        ...
        cashMethod = await PaymentMethod_1.default.findOrCreate({ adapter: paymentMethodSeed.adapter }, paymentMethodSeed);
        let result = await PaymentMethod_1.default.getAdapter(cashMethod.adapter);
    ...
    it("getAdapterById", async function () {
        ...
        let error = null;
        try {
            await PaymentMethod_1.default.getAdapterById(cashMethod.id);
        }
        catch (e) {
            error = e;
        }
    ...
    it("isPaymentPromise", async function () {
        ...
        //
        cashMethod = await PaymentMethod_1.default.findOrCreate({ adapter: paymentMethodSeed.adapter }, paymentMethodSeed);
        let result1 = await PaymentMethod_1.default.isPaymentPromise(cashMethod.id);
        ...
        let externalPaymentMethod = await PaymentMethod_1.default.findOne({
            adapter: "test-payment-system",
        });
        let result2 = await PaymentMethod_1.default.isPaymentPromise(externalPaymentMethod.id);
