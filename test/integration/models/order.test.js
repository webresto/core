...
describe("Order", function () {
    ...
    it("check model fields", async function () {
        ...
        order = await Order_1.default.findOne(order.id).populate("dishes");
    ...
    it("addDish", async function () {
        order = await Order_1.default.create({ id: "add-dish" }).fetch();
        await Order_1.default.addDish({ id: order.id }, dishes[0], 1, [], "", "user");
        await Order_1.default.addDish({ id: order.id }, dishes[1], 5, [], "test comment", "user");
        // /    await Order.addDish({id: order.id}, dishes[1], 5, [], "", "user");
        //await Order.addDish({id: order.id}, dishes[1], 5, [], "test comment", "user");s
        let result = await Order_1.default.findOne(order.id).populate("dishes");
        (0, chai_1.expect)(result.dishes.length).to.equal(2);
        let orderDish = await OrderDish_1.default.find({
            order: order.id,
            dish: dishes[0].id,
        }).sort("createdAt ASC");
        ...
        orderDish = await OrderDish_1.default.find({ order: order.id, dish: dishes[1].id }).sort("createdAt ASC");
    ...
    it("removeDish", async function () {
        let dish = (await Order_1.default.findOne(order.id).populate("dishes")).dishes[1];
        dish = await OrderDish_1.default.findOne(dish.id);
        await Order_1.default.removeDish({ id: order.id }, dish, 1, false);
        let changedDish = await OrderDish_1.default.findOne(dish.id);
        (0, chai_1.expect)(changedDish.amount).to.equal(dish.amount - 1);
    });
    it("addDish same dish increase amount", async function () {
        order = await Order_1.default.create({ id: "adddish-same-dish-increase-amount-1" }).fetch();
        await Order_1.default.addDish({ id: order.id }, dishes[0], 2, [], "", "user");
        await Order_1.default.addDish({ id: order.id }, dishes[0], 3, [], "", "user");
        await Order_1.default.addDish({ id: order.id }, dishes[0], 1, null, "", "user");
        let orderDishes = await OrderDish_1.default.find({ order: order.id, dish: dishes[0].id });
        (0, chai_1.expect)(orderDishes.length).to.equals(1);
        (0, chai_1.expect)(orderDishes[0].amount).to.equals(6);
        order = await Order_1.default.create({ id: "adddish-same-dish-increase-amount-2" }).fetch();
        await Order_1.default.addDish({ id: order.id }, dishes[0], 1, [{ id: dishes[1].id, modifierId: dishes[1].id }], "", "user");
        await Order_1.default.addDish({ id: order.id }, dishes[0], 1, null, "", "user");
        await Order_1.default.addDish({ id: order.id }, dishes[0], 2, null, "", "user");
        orderDishes = await OrderDish_1.default.find({ order: order.id, dish: dishes[0].id });
    ...
    it("setComment", async function () {
        let dish = (await Order_1.default.findOne({ id: order.id }).populate("dishes")).dishes[0];
        dish = await OrderDish_1.default.findOne({ id: dish.id });
        let testComment = "this is a test comment";
        await Order_1.default.setComment({ id: order.id }, dish, testComment);
        let changedDish = await OrderDish_1.default.findOne({ id: dish.id });
        (0, chai_1.expect)(changedDish.comment).to.equal(testComment);
    });
    it("addDish 20", async function () {
        order = await Order_1.default.create({ id: "adddish-20" }).fetch();
        for (let i = 0; i < 20; i++) {
            await Order_1.default.addDish({ id: order.id }, dishes[i], 3, [], "", "user");
        }
    });
    it("addDish 21th", async function () {
        await Order_1.default.addDish({ id: order.id }, dishes[21], 3, [], "", "user");
    });
    it("setSelfService", async function () {
        let order = await Order_1.default.create({ id: "setselfservice" }).fetch();
        order = await Order_1.default.setSelfService({ id: order.id }, true);
        (0, chai_1.expect)(order.selfService).to.equal(true);
        order = await Order_1.default.setSelfService({ id: order.id }, false);
        (0, chai_1.expect)(order.selfService).to.equal(false);
    });
    it("countCart", async function () {
        let order = await Order_1.default.create({ id: "test--countcart" }).fetch();
        await Order_1.default.updateOne({ id: order.id }, { user: "user" });
        await Order_1.default.addDish({ id: order.id }, dishes[0], 5, [], "", "user");
        await Order_1.default.addDish({ id: order.id }, dishes[1], 3, [], "", "user");
        await Order_1.default.addDish({ id: order.id }, dishes[2], 8, [], "", "user");
        ...
        // // Modifier with price
        await Order_1.default.addDish({ id: order.id }, dishes[5], 1, [{ id: dishes[6].id, modifierId: dishes[6].id }], "", "user");
        let changedOrder = await Order_1.default.countCart({ id: order.id });
    ...
    it("order", async function () {
        ...
        await Order_1.default.setSelfService({ id: order.id }, true);
        await Order_1.default.check({ id: order.id }, customer_1.customer, true, undefined, undefined);
        await Order_1.default.order({ id: order.id });
    ...
    it("paymentMethodId", async function () {
        let order = await Order_1.default.create({ id: "paymentmethodid" }).fetch();
        let testPaymentSystem = await ExternalTestPaymentSystem_1.default.getInstance();
        let paymentSystem = (await PaymentMethod.find())[0];
        await Order_1.default.update({ id: order.id }, { paymentMethod: paymentSystem.id }).fetch();
        let result = await Order_1.default.paymentMethodId({ id: order.id });
        (0, chai_1.expect)(result).to.equal(paymentSystem.id);
    });
    it("doPaid", async function () {
        (0, chai_1.expect)(Order_1.default.doPaid).to.not.equals(undefined);
        let order = await Order_1.default.create({ id: "dopaid" }).fetch();
        await Order_1.default.addDish({ id: order.id }, dishes[0], 5, [], "", "user");
        await Order_1.default.addDish({ id: order.id }, dishes[1], 3, [], "", "user");
        await Order_1.default.addDish({ id: order.id }, dishes[2], 8, [], "", "user");
        await Order_1.default.check({ id: order.id }, customer_1.customer, true, undefined, undefined);
        ...
        var paymentDocument = await PaymentDocument_1.default.create(newPaymentDocument).fetch();
        await Order_1.default.doPaid({ id: order.id }, paymentDocument);
