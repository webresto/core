    ...
    try {
      ...
      returnFullCart(cart).then(cart => {
        fs.readFile(process.cwd() + path, function (err, data) {
          if (err) return sails.log.error('err email1', err);
          ...
          cart.op = orderProblem;
            ...
            let html;
            try {
                html = ejs.render(data.toString(), cart);
            } catch (e) {
                sails.log.error('fail while render EJS', cart, e);
            }

            if (!cart.iikoId) {
                sails.log.error('cart has not iikoId', cart);
            }

          const message = {
            from: sails.config.restocore.email.server.user,
            to: emails,
              subject: 'Заказ №' + cart.iikoId,
            attachment:
              {
                data: html,
                alternative: true
          ...
          server.send(message, function (err, message) {
            if (err) return sails.log.error('err email2', err);
      ...
      return sails.log.error('err email3', err);
    }
  }
});
...
module.exports.fail = async function (cart) {
  try {
    ...
    returnFullCart(cart).then(cart => {
      fs.readFile(process.cwd() + path, function (err, data) {
        if (err) return sails.log.error('err email4', err);
        ...
        server.send(message, function (err, message) {
          if (err) return sails.log.error('err email5', err);
    ...
    return sails.log.error('err email6', err);
  }
};
