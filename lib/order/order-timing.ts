...
export interface DeliveryTimeInput {
   ...
   *
   * Core never reads a zone to find it: where the floor comes from is the
   * adapter's business, and asking here would mean core knowing about
   * `DeliveryZone`.
   */
 ...
 * straight-line estimate of eleven minutes does not overrule them. No floor
 * reported means none: the road alone.
 */
...
/**
 * Whether one product alone can be cooked inside the customer's ceiling.
 *
 * Cooking only: the road is not known product by product, and the whole promise
 * is judged at checkout. A product with no time configured passes — no number,
 * no promise, as in `getPreparationMinutes`. The menu hides what fails this and
 * the basket refuses it, by this one rule.
 */
export function productFitsMaxWait(
  product: AvailabilityProduct,
  maxWaitMinutes: number | null | undefined,
): boolean {
  return fitsMaxWait({ totalMinutes: getPreparationMinutes([product]) }, maxWaitMinutes);
}
