...
// Nor as "delivery is charged as a cost, with no product line".
...
/**
 * Whether a row has to state its own delivery time and cost. Only a zone in a
 * layer that lends its terms is spared; the model refuses the rest without
 * them. A layer never has a parent, so it always prices itself.
 */
function pricesItself(row, zones) {
  if (!row.parent) return true;
  return zones.find((other) => other.id === row.parent)?.termsApplyToZones === false;
}

/**
 * Products to charge delivery as: the catalog's first page, plus the one already
 * chosen when it is not on it — otherwise the select would show that zone's
 * product as nothing.
 */
function useDeliveryProducts(currentId) {
  const [dishes, setDishes] = useState([]);

  useEffect(() => {
    let cancelled = false;
    (async () => {
      const { ok, payload } = await api('/core/marketing/dishes');
      let list = ok ? payload.results || [] : [];
      if (currentId && !list.some((dish) => dish.id === currentId)) {
        const chosen = await api(`/core/marketing/dishes?ids=${encodeURIComponent(currentId)}`);
        if (chosen.ok) list = [...(chosen.payload.results || []), ...list];
      }
 ...
 * that has a shape — by double-clicking it on the map. It is where these fields
 * are edited; the editing strip carries only the two a row cannot be saved
 * without.
 *
...
function TariffDialog({ zone, layerName, layerTerms, canManage, onSave, onClose, saving, t }) {
  ...
  const deliveryProducts = useDeliveryProducts(zone.deliveryItem || null);
            ...
            {!inherited && (
              <>
                ...
                <div style={styles.field}>
                  ...
                  <Select
                    value={draft.deliveryItem || NO_PRODUCT}
                    disabled={!editable}
                    onValueChange={(value) => setField('deliveryItem', value === NO_PRODUCT ? '' : value)}
                  >
                    <SelectTrigger><SelectValue /></SelectTrigger>
                    <SelectContent className={DROPDOWN_OVER_DIALOG}>
                      <SelectItem value={NO_PRODUCT}>{t('No delivery product')}</SelectItem>
                      {deliveryProducts.map((dish) => (
...
/**
 * A map file, and the two terms every zone in it is created with.
 *
 * A file carries no layers, so each of its zones prices itself, and the model
 * refuses one without a time and a cost. The rest of the terms are set zone by
 * zone afterwards, as for a zone drawn here. Mounted only while open, like the
 * terms popup, so a finished upload closes it by unmounting.
 */
function UploadZonesDialog({ onUpload, onClose, t }) {
  const [minDeliveryTime, setMinDeliveryTime] = useState('');
  const [deliveryCost, setDeliveryCost] = useState('');
  const ready = minDeliveryTime !== '' && deliveryCost !== '';

  const handleOpenChange = useCallback((isOpen) => { if (!isOpen) onClose(); }, [onClose]);

  const upload = async (content) => {
    const done = await onUpload(content, { minDeliveryTime: Number(minDeliveryTime), deliveryCost: Number(deliveryCost) });
    if (done) onClose();
  };
  ...
  return (
    <DialogStack open onOpenChange={handleOpenChange}>
      ...
      <DialogStackBody>
        <DialogStackContent>
          <DialogStackHeader>
            <DialogStackTitle>{t('Zones from a file')}</DialogStackTitle>
            <DialogStackDescription>
              {t('Every zone in the file is created with these terms. The rest are set per zone afterwards.')}
            </DialogStackDescription>
          </DialogStackHeader>

          <div className="flex flex-col p-6" style={{ gap: 14 }}>
            <div style={styles.field}>
              <Label style={styles.fieldLabel}>{t('Delivery time, minutes')}</Label>
              <Input type="number" value={minDeliveryTime} onChange={(event) => setMinDeliveryTime(event.target.value)} />
            </div>
            <div style={styles.field}>
              <Label style={styles.fieldLabel}>{t('Delivery cost')}</Label>
              <Input type="number" value={deliveryCost} onChange={(event) => setDeliveryCost(event.target.value)} />
            </div>
          </div>
...
function SourceSettingsDialog({ cityId, cityName, canManage, onSaved, t }) {
  ...
  const load = useCallback(async () => {
    ...
    setMinDeliveryTime(String(entry?.minDeliveryTime ?? ''));
    setDeliveryCost(String(entry?.deliveryCost ?? ''));
        ...
        <DialogStackContent>
          ...
          <div className="flex flex-col p-6" style={{ gap: 14, overflowY: 'auto' }}>
            ...
            <div style={styles.field}>
              ...
              <span style={styles.help}>
                {t('Required with a map link. New layers and zones from the map are created with these terms; zones already here keep theirs.')}
              </span>
            </div>
...
function DeliveryZonesContent({ canManage }) {
  ...
  const uploadZones = useCallback(async (content, terms) => {
    const { ok, payload } = await api('/core/delivery-zones-upload', {
      method: 'POST',
      body: JSON.stringify({ city: cityId || '', content, ...terms }),
    });
    if (!ok) {
  ...
  // The same kind of refusal for the terms the model requires.
  const ownTerms = Boolean(selected) && pricesItself(selected, cityZones);
  const needsTerms = ownTerms && (selected.minDeliveryTime === '' || selected.deliveryCost === '');
  ...
  return (
    <div className="absolute inset-0 overflow-hidden" style={{ background: 'var(--background)', color: 'var(--foreground)' }}>
          ...
          {canManage && (
            <div style={{ display: 'flex', gap: 6, marginBottom: 10 }}>
              <Button
                ...
                style={{ flex: 1, minWidth: 0, justifyContent: 'flex-start' }}
              >
                <MaterialIcon name="upload_file" size={14} style={{ marginRight: 4 }} />
                {t('Zones from a file')}
              </Button>
            </div>
          )}
        ...
        <main style={{ flex: 1, minWidth: 0, minHeight: 0, display: 'flex', flexDirection: 'column' }}>
          ...
          {editing && selected && (
            <div style={{ padding: 12, borderBottom: '1px solid var(--border)', display: 'flex', gap: 12, alignItems: 'flex-end', flexWrap: 'wrap' }}>
              ...
              {/* The two terms the row cannot be saved without, next to Save:
                  a new row has no terms popup yet, and a zone taken out of a
                  lending layer has nowhere else to get them. */}
              {ownTerms && (
                <>
                  <div style={{ ...styles.field, width: 150 }}>
                    ...
                    <Input
                      type="number"
                      value={selected.minDeliveryTime}
                      disabled={!canManage}
                      onChange={(event) => setDraft((prev) => ({ ...prev, minDeliveryTime: event.target.value }))}
                    />
                  </div>
                  <div style={{ ...styles.field, width: 150 }}>
                    <Label style={styles.fieldLabel}>{t('Delivery cost')}</Label>
                    <Input
                      ...
                      disabled={!canManage}
                      onChange={(event) => setDraft((prev) => ({ ...prev, deliveryCost: event.target.value }))}
                    />
                  </div>
                </>
              )}
              ...
              <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
                {needsTerms && (
                  <span style={{ ...styles.help, maxWidth: 260, color: 'var(--destructive)' }}>
                    {t('Fill in the delivery time and cost: no layer prices this row for it.')}
                  </span>
                )}
                {needsShape && (
                  <span style={{ ...styles.help, maxWidth: 260, color: 'var(--destructive)' }}>
                ...
                <Button onClick={onSaveDraft} disabled={saving || !canManage || needsShape || needsTerms}>
                  {saving ? t('Saving…') : t('Save')}
                </Button>
      ...
      {uploadOpen && <UploadZonesDialog onUpload={uploadZones} onClose={closeUpload} t={t} />}
