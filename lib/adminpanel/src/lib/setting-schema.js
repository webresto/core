 ...
 */
export function validateValueBySchema(schema, value, t, patternHint) {
  ...
  if (typeof value === 'string' && isString) {
    ...
    if (typeof schema.pattern === 'string') {
      ...
      if (!regexp.test(value)) {
        // A raw regexp is not an instruction. When the manifest says what the rule means
        // ("lowercase latin letters, digits and hyphens"), show that instead.
        return patternHint
          ? `${t('Value must contain only')}: ${t(patternHint)}`
          : `${t('Value must match the pattern')}: ${schema.pattern}`;
      }
    }
    return null;
  }
