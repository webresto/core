...
export default async function UpsertDeliveryZoneController(req: any, res: any) {
  try {
    ...
    if (id) {
      ...
      // The id rides along for the model: whether the terms are required
      // depends on the row, and the update hook sees only these values.
      const updated = await DeliveryZone.updateOne({ id }, { ...(locked ? pickOperatorFields(values) : values), id });
