...
interface CitySource {
  ...
  /** What the source's new layers and loose zones are created with. */
  minDeliveryTime: number | null;
  deliveryCost: number | null;
}

function termOf(value: unknown): number | null {
...
function readCities(config: Record<string, any>): CitySource[] {
  ...
  if (!Array.isArray(cities)) {
    ...
    return url
      ? [{
        city: "",
        url,
        externalIdSource: idSourceOf(config, config),
        minDeliveryTime: termOf(config.minDeliveryTime),
        deliveryCost: termOf(config.deliveryCost),
      }]
      : [];
  }
  ...
  return cities
    ...
    .map((entry: any) => ({
      ...
      // Read the way the sync reads them: the city's own, else the shared key.
      minDeliveryTime: termOf(entry.minDeliveryTime ?? config.minDeliveryTime),
      deliveryCost: termOf(entry.deliveryCost ?? config.deliveryCost),
    }));
}
