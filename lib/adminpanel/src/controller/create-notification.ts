...
export default async function CreateNotificationController(req: any, res: any) {
  ...
  try {
    ...
    // Direct device send (cart/guest device): addressed as "user" delivery,
    // but without requiring userId; push goes to a specific UserDevice.
    ...
    if (normalizedDeviceId) {
      // Target is a cart/device: send to a specific UserDevice by id.
      ...
      // If the device is bound to a user, use that user for history/links.
