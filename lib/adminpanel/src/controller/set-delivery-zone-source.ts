...
export default async function SetDeliveryZoneSourceController(req: any, res: any) {
  try {
    ...
    // What every new self-priced row of this source is created with: the model
    // refuses a layer or a loose zone without them, so a link without them
    // would import nothing but failures.
    const terms = readZoneTerms(req.body);
    if (url && !terms) {
      return res.status(400).json({ error: "Delivery time and delivery cost are required for a map link" });
    }
    ...
    if (url) {
      ...
      const entry = { city, url, externalIdSource: idSource, ...terms };
