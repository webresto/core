...
/**
 * The time and cost new self-priced rows are created with, as the upload form
 * and the source popup send them — or `null` when either is missing or
 * negative.
 */
export function readZoneTerms(body: any): ZoneTerms | null {
  const minDeliveryTime = toNumberOrNull(body?.minDeliveryTime);
  const deliveryCost = toNumberOrNull(body?.deliveryCost);
  if (minDeliveryTime === null || minDeliveryTime < 0 || deliveryCost === null || deliveryCost < 0) return null;
  return { minDeliveryTime, deliveryCost };
}
