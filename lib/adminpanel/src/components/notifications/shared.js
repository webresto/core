...
// Reactively tracks whether the viewport is at/below `breakpoint` px (mobile).
// Used to collapse the multi-column inline-style grids into a single column.
...
// notificationsApi(path, opts) -> { ok, status, payload } on top of the adminizer client.
...
// Toast helper around window.sonner. A <Toaster/> has to be mounted — see ModuleToaster.
...
/**
 * The <Toaster/> of a module page.
 *
 * Adminizer mounts one inside every page component it ships (list, catalog, add,
 * dashboard, login) and not in the layout — and the page that renders a module
 * (its `assets/module-*.js`) is not one of them. Without a Toaster sonner still
 * takes the toast and keeps it: `subscribe()` replays the active ones to the first
 * Toaster that mounts, so the message surfaces later, on whatever host page the
 * operator opens next.
 *
 * Mount once at the root of a module page, with the props the host uses.
 */
export function ModuleToaster() {
  return React.createElement(window.UIComponents.Toaster, {
    position: 'top-center',
    richColors: true,
    closeButton: true,
  });
