...
// lib/adminpanel/controls/modifiersEditorHelper.ts so the form validates the exact
// same invariants the runtime (lib/ProductModifier.ts) enforces. Kept dependency-free
...
// ephemeral `_id` for stable React keys and an `extra` bag preserving unknown keys.

let SEQ = 0;
export const nextId = () => `mod-${Date.now().toString(36)}-${(SEQ += 1)}`;

const CHILD_KNOWN = new Set([
  'id', 'rmsId', 'minAmount', 'maxAmount', 'defaultAmount', 'freeOfChargeAmount', 'required',
]);
const GROUP_KNOWN = new Set([
  'id', 'rmsId', 'minAmount', 'maxAmount', 'required', 'freeOfChargeAmount',
