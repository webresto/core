 ...
 */
function mergeValues(existing: Partial<DishPlaceValues>, values: DishPlaceValues): DishPlaceValues {
  return {
...
let Model = {
   ...
   */
  async upsertForPlace(
    dish: string,
    place: string,
    values: DishPlaceValues,
  ): Promise<DishPlaceRecord | null> {
    const existing = await DishPlace.findOne({ dish, place });
    if (existing) {
