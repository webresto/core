 ...
 */
const REQUIRED_TERMS = ["minDeliveryTime", "deliveryCost"] as const;

/** What an update has to write for the terms rule to be asked again. */
const TERMS_RULE_FIELDS = ["parent", ...REQUIRED_TERMS] as const;
 ...
 */
async function assertTerms(record: Partial<DeliveryZoneRecord>): Promise<void> {
  const parentId = toId(record.parent);
  const layer = parentId ? await findZone(parentId) : null;
  if (layer && layer.termsApplyToZones !== false) return;

  const missing = REQUIRED_TERMS.filter((field) => !Number.isFinite(record[field]));
  if (missing.length) {
    throw new Error(
      `DeliveryZone "${record.name || record.id || ""}" needs ${missing.join(" and ")}: ` +
  ...
  async beforeUpdate(values: Partial<DeliveryZoneRecord>, cb: (err?: string) => void) {
    try {
      ...
      if (TERMS_RULE_FIELDS.some((field) => field in values)) {
        if (!values.id) throw new Error("A DeliveryZone update that writes its layer or its terms has to carry the row id");
