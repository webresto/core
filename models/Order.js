...
const cooking_place_1 = require("../lib/cooking-place");
...
const route_assignment_1 = require("../lib/route-assignment");
...
let Model = {
    ...
    async countCart(criteria, isPromoting = false) {
        ...
        try {
            ...
            if (kitchen.changed)
                order.cookingPoint = kitchen.placeId;
            if (kitchen.changed) {
            ...
            for (const orderDish of liveLines) {
                ...
                const linePlace = routed ?? (0, cooking_place_1.toPlaceId)(order.cookingPoint);
                if (routed && (0, cooking_place_1.toPlaceId)(orderDish.cookingPoint) !== routed) {
                    orderDish.cookingPoint = routed;
                    await OrderDish.update({ id: orderDish.id }, { cookingPoint: routed }).fetch();
                }
                if (linePlace && !routePoints.includes(linePlace))
                    routePoints.push(linePlace);
            }
