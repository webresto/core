...
let attributes = {
    ...
    worktime: "json",
    /**
     * The four commercial numbers, and all four are nullable columns.
     *
     ...
     * a zone that a layer prices.
     *
     * Two of them are not optional for a row that prices itself: `minDeliveryTime`
     * and `deliveryCost`. Whether a row prices itself depends on its layer, so
     * that is enforced by `assertTerms` in the lifecycle hooks and not by
     * `required` here.
     *
     * They came from the legacy schema without `allowNull`, and Waterline refuses
     * null on a plain `number`. The editor posts null for an empty field.
     */
...
async function assertZone(record) {
    ...
    if (hasShape && !(0, zone_match_1.isValidPolygon)(record.polygon)) {
...
/**
 * The terms a row cannot go without unless its layer lends them.
 *
 * The other three are optional by nature — not every zone has a minimum order
 * or a threshold for free delivery. A time and a cost are not: without them a
 * zone promises nothing and charges nothing, which is no tariff at all.
 */
const REQUIRED_TERMS = ["minDeliveryTime", "deliveryCost"];
/** What an update has to write for the terms rule to be asked again. */
const TERMS_RULE_FIELDS = ["parent", ...REQUIRED_TERMS];
/**
 * A row states its own time and cost unless the layer above it lends its
 * terms: a loose zone, a layer, and a zone in a layer that only groups all
...
async function assertTerms(record) {
    ...
    if (layer && layer.termsApplyToZones !== false)
        return;
    const missing = REQUIRED_TERMS.filter((field) => !Number.isFinite(record[field]));
    if (missing.length) {
        throw new Error(`DeliveryZone "${record.name || record.id || ""}" needs ${missing.join(" and ")}: ` +
            (layer ? "its layer leaves the terms to each zone" : "it has no layer to take them from"));
    }
}
/**
...
let Model = {
    ...
    async beforeUpdate(values, cb) {
        try {
            ...
            // Waterline hands this hook the values and not the row, and whether the
            // terms are required depends on fields an update need not carry — the
            // locked editor writes the terms without the parent. So an update that
            // writes either is judged as the row will be once written, and has to say
            // which row that is.
            if (TERMS_RULE_FIELDS.some((field) => field in values)) {
                if (!values.id)
                    throw new Error("A DeliveryZone update that writes its layer or its terms has to carry the row id");
                await assertTerms({ ...(await findZone(values.id)), ...values });
            }
            cb();
        }
        catch (error) {
    ...
    async afterCreate(record, proceed) {
        (0, zone_cache_1.invalidateDeliveryZoneCache)();
        proceed();
    },
    ...
    async findServing() {
        ...
        return zones
            .filter((zone) => (0, zone_match_1.isValidPolygon)(zone.polygon))
            .filter((zone) => {
            if (zone.enable === false)
                return false;
            const layer = layerOf(zone);
