...
let attributes = {
  ...
  */
  ...
  type: "string" as unknown as ProductType,
...
let Model = {
  beforeCreate: async function (init: DishRecord, cb: (err?: string) => void) {
    ...
    if (!PRODUCT_TYPES.includes(init.type as ProductType)) {
   ...
   * @return Found dishes
   */
  async getDishes(criteria: any = {}, placeIds?: string[], order?: MenuRequest["order"]): Promise<DishRecord[]> {
    ...
    const context = placeIds === undefined
      ...
      : { placeIds, source: "requested" as const, placeRequired: false, diagnostics: [] };
   ...
   */
  async getDishModifiers(dish: DishRecord, placeIds?: string[]): Promise<DishRecord> {
  ...
  getRecommended: async function (ids: string[], limit = 12, includeReverse = false, placeIds?: string[]): Promise<DishRecord[]> {
