...
export class PromotionAdapter extends AbstractPromotionHandlerINSTANCE {
  ...
  public static async filterPromotions(promotionsByConcept: Promotion[], target: Group | Dish | Order): Promise<Promotion[] | undefined> {
    ...
    let filteredByJointPromotions: Promotion[] | undefined[] = [filteredByCondition.find((promotion) => {
    ...
    if (order.user && typeof order.user === "string") {
        ...
        for (const orderDish of orderDishes) {
          ...
          if(spendDiscount.discountType === "percentage") {
            ...
            orderDishDiscountCost = new Decimal(orderDish.dish.price).mul(orderDish.amount).mul(+spendDiscount.discountAmount / 100).toNumber()
            discountCost = new Decimal(orderDishDiscountCost).add(discountCost)
          }
