 ...
 *       "minDeliveryTime": 60, "deliveryCost": 200 } ] }
 *
 * `minDeliveryTime` and `deliveryCost` are what a new layer or a new zone
 * outside a lending layer is created with — see `ZoneTerms`.
 *
...
export class DeliveryZoneSyncService {
  ...
  private static async executeCity(
    ...
    try {
      ...
      // The terms for new rows sit there too, next to the link they came with.
      const result = await DeliveryZoneImportService.apply(
        { ...snapshot, city: target.city },
        {
          dryRun,
          terms: { minDeliveryTime: target.config.minDeliveryTime, deliveryCost: target.config.deliveryCost },
        },
      );
