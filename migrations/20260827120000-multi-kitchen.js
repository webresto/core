  ...
  async.series([
    ...
    (cb) => db.createTable('address', {
      columns: {
        ...
        // A `range` node stands for a span of house numbers — "1 to 99" — so a
        // street of five hundred houses is one row instead of five hundred.
    ...
    // --- userlocation: an order's address, kept ----------------------------
    //
    // A saved location is the address of a delivered order without its catalog
    // node: the line, the coordinate and the tail. It points at nothing, so a
    // catalog edit cannot change where it is. Nothing ever filled `index`.
    (cb) => db.removeColumn('userlocation', 'street', cb),
    (cb) => db.removeColumn('userlocation', 'index', cb),
    (cb) => db.addColumn('userlocation', 'formatted', { type: 'text', notNull: false }, cb),
    (cb) => db.addColumn('userlocation', 'coordinate', { type: 'json', notNull: false }, cb),

    // --- order: which kitchens, how long the customer will wait ------------
    //
    // `cookingPoints` is json rather than a join table. It is an ordered list of
    // kitchens belonging to one order, the assigned one first — read whole,
    // written whole, never queried by member.
    ...
    // stores a singular association as the foreign primary key, and this schema
    // declares no constraint for those.
...
exports.down = function (db, callback) {
  async.series([
    ...
    (cb) => db.removeColumn('userlocation', 'coordinate', cb),
    (cb) => db.removeColumn('userlocation', 'formatted', cb),
    (cb) => db.addColumn('userlocation', 'index', { type: 'text', notNull: false }, cb),
    (cb) => db.addColumn('userlocation', 'street', { type: 'text' }, cb),
